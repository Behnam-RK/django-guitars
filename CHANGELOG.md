...
- Fixed: an arm reading the *dependent's own* table excludes the target row, by the rule's key. `tests/testapp`'s `Placard` now owns itself and `Ensemble` owns the `PressKit` its `Orchestra` child owns, so both exclusion shapes ship in generated migrations and are asserted against Postgres rather than as rendered text. A target declaring an `OwningForeignKey` to itself is refused a rule of its own — that relation is a one-node `ON UPDATE` cycle — but still contributes an arm to every other owner's rule, and a row of it pointing at its own primary key read as its own live owner, pinning the target un-archivable by any owner forever.
