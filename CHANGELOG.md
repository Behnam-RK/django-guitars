...
- Fixed: an arm reading the *dependent's own* table excludes the target row, by the rule's key. `tests/testapp` carries all four exclusion shapes in generated migrations, asserted against Postgres rather than as rendered text: `Placard` owns itself, `Ensemble` owns the `PressKit` its `Orchestra` child owns, and `SpotlitPlacard` is an MTI child of `Placard` owning it back. A target declaring an `OwningForeignKey` to itself is refused a rule of its own — that relation is a one-node `ON UPDATE` cycle — but still contributes an arm to every other owner's rule, and a row of it pointing at its own primary key read as its own live owner, pinning the target un-archivable by any owner forever.
