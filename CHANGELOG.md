...
- Fixed: an arm reading the *dependent's own* table excludes the target row, by the rule's key. A target declaring an `OwningForeignKey` to itself is refused a rule of its own — that relation is a one-node `ON UPDATE` cycle — but still contributes an arm to every other owner's rule, and a row of it pointing at its own primary key read as its own live owner, pinning the target un-archivable by any owner forever.
- Fixed: self-exclusion is keyed on the **row** rather than the column. Every arm on the table the rule fires on excludes `old."<pk>"`; arms on other tables carry none, no row of theirs going away in that statement. Excluding only the declaring column's arm would have let a row owning the target through two of its own columns read as its own last live owner and hold the target alive forever — the shape `tests/testapp`'s `Album.press_kit`/`Album.alt_press_kit` pair has always had.
...
- Added: `_scoped_owned_gap_notes`, the owned twin of the cascade gap notes. A scoped run whose in-scope models move an out-of-scope owned rule's guard now says so on stdout. Warned rather than escalated: an unscoped run — what CI runs — re-derives every rule and emits the `DROP`+`CREATE`. It re-runs the owned candidate test over apps the run was *not* asked about, so it asks for that test's verdict without its warnings — otherwise a scoped run would report another app's misconfigurations, and fail `--check` over one of them.
