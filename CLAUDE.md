...
- Rule guards are `<> 'on'`, never `= 'off'` — a rolled-back `set_config` reads back as the empty string, and `= 'off'` then failed toward *destroying* data. See `docs/soft-deletion.md`. `guitars.models.OwningForeignKey` is the kit's only custom field: its `deconstruct()` path is **frozen** (migrations record it literally), and its rule's last-owner `NOT EXISTS` guard is unconditional, never derived from a `UniqueConstraint` — dropping a constraint changes no field, so the `[SQL:...]` identity would not move and `--check` would stay green over a rule gone wrong. `hard_delete()` re-implements that predicate in Python, strictly narrower (it removes a row where the rule stamps a column); the two change together. Which relations carry a rule at all is one shared answer — `introspection.rule_update_cycle_edges` over the **whole** registry — read by the generator *and* by `hard_delete()`: an `ON UPDATE` rule cycle bricks every table in it (PostgreSQL rejects every `UPDATE`, guard unread), so every edge on one is refused, and following in Python what the generator refused destroys what the rule spared. See [ADR 0011](docs/adr/0011-owner-side-soft-delete-ownership.md).
