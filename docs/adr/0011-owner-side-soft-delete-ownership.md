...
- A relation whose target's `_deleted_at` resolves to the owner's *own* table — self-ownership, or ownership of an MTI descendant — is refused with a warning. The rule's action would update the table it fires on, and PostgreSQL rewrites such a rule into itself, then rejects every `UPDATE` on the table (a plain `save()` included) as infinite rule recursion. `hard_delete()` re-implements that refusal for the same reason it re-implements the others.
