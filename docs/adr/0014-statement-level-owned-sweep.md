...
- **The hole is permanent, not deferred.** Once the last owner is archived nothing ever stamps the
  dependent again. "Fails safe" holds of the *direction* — a leaked live row rather than an archived
  owned one — but the state is wrong for ever, and a dependent mirroring an external resource leaves
  that side unreclaimed.
- **Ordinary code walks into it.** `QuerySet.delete()` is how Django deletes, nothing at the
  declaration site tells it from a loop, and `Collector` batches by model on its own, so a third
  model's cascade reaches the same statement with nobody writing a queryset delete.
...
- **The arms are spliced in verbatim**, the archived owners aliased into a subquery the templates
  ...
  skipped the held one, and broke the subset.
...
- **`sweepowned` repairs what is already lost**, the trigger reaching no database that leaked
  ...
  and since 2.7.0 it repairs to a **fixpoint**: a pass walks dependents in model-label order, so a
  chain sorting against that order is left a hop short, the repairing `UPDATE` firing the rule but
  not the trigger such a database lacks. Bounded by the dependent count; exceeding the bound is
  reported as "did not settle" rather than diagnosed — concurrency and a rule cycle look alike.
...
no column to read liveness from, and would pin the dependent un-archivable for ever.
...
**Accepted costs.**
- A new operation family: header, scanner, container, name family, corpus baseline — the bulk of
  the change, smaller than 0012 estimated: the owned SQL templates were private already.
- One `CREATE FUNCTION`/`CREATE TRIGGER` per owned relation, and a statement-level trigger per `UPDATE` on an owner table, matching nothing where the rule already stamped.
- A `db_table` or column containing `$$` is **refused**, not escaped: it closes the body's dollar
  quoting and `migrate` would fail. Only the sweep is skipped, so only a recorded one is named to
  drop by hand.
...
- `_updated_at` is stamped only where the dependent's **own** table holds it. The one shape putting
  it on an MTI ancestor is refused outright since 2.7.0 ([ADR 0015](0015-refuse-soft-deletable-mti-orphans.md)), so the sweep never meets one.

**Reversibility.** Low, as 0011 and 0012 describe: the SQL is inlined, so a database keeps what it was
migrated with until a regeneration and a `migrate`, and dropping the family leaves every migrated project's triggers live, no command retiring one.
