...
| [`0021`](0021-retirement-ordered-against-its-create.md) | why a cascade retirement now carries an edge to the migration that created the rule it drops, why the scan records that node rather than resolving it, and why `--check` names a history already written (2.10.0) |
| [`0022`](0022-router-gated-enforcement.md) | why the generator now asks the database router which models its PostgreSQL DDL applies to, why `allow_migrate_model` rather than `db_for_write`, and why three registry walks are deliberately left ungated (2.11.0) |
| [`0023`](0023-cascade-stamps-its-parents-timestamp.md) | why a cascade now copies its parent's `_deleted_at` instead of calling `NOW()`, why it skips a child already archived, and why the frozen public constants keep the old body (2.11.0) |
| [`0024`](0024-inverse-cascade-revive-rules.md) | why reviving a parent now revives the children that archive took, why the archive timestamp is the provenance test, why the inverse is a statement trigger rather than a second rule, and why the owned and self-referential families stay archive-only (2.11.0) |
