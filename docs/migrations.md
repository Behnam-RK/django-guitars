...
PostgreSQL dedupes a rule on its name per table, not on what it references, so two operations resolving to one name is a **silent replacement** rather than an error — the second wins, the first relation stops cascading, and `--check` stays green over the loss. Every name folds a schema-qualified table in **length-prefixed** (`('tenant_a', 'events')` and `('tenant', 'a_events')` would otherwise meet). The cascade family adds its FK column plainly: an inbound child with two `CASCADE` foreign keys to one parent keeps the bare `soft_delete_related_<child>` for the first and takes `soft_delete_related_<child>_<fk>` for the rest, which a child table named exactly another child's `<table>_<column>` can claim twice. That spelling shipped in 0.x and **cannot move** — nothing retires a rule, so renaming it would leave every migrated project's old rule live beside the new one — so the generator reports the clash on stderr instead of renaming around it. The *owned* family, new in 2.3.0 and free of that history, is unambiguous by construction: `soft_delete_owned_<target>_<n>_<fk>`, where `n` is the column name's length. The two families' distinct prefixes are what keep them from meeting at all.
