...
* **Idempotency has three layers** (see ``docs/migrations.md``), and this module owns
  only the first: the content digest stamped on the migration's first line
  (``[DIGEST:...]``), identifying an unchanged operation set. The other two -- the
  per-operation comment header and its ``[SQL:...]`` identity -- are each command's own,
  not shared here.
